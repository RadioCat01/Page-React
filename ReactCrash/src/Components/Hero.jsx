...
const Hero = () => {
  return (
    <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      {/* Floating shapes */}
      <div className="absolute top-1/4 left-[15%] w-3 h-3 rounded-full bg-orange-500/40 animate-float" />
      <div className="absolute top-1/3 right-[20%] w-2 h-2 rounded-full bg-orange-600/40 animate-float-delayed" />
      <div className="absolute bottom-1/3 left-[25%] w-4 h-4 rounded-full bg-amber-400/30 animate-float" style={{ animationDelay: '1s' }} />
      <div className="absolute top-2/3 right-[30%] w-2.5 h-2.5 rounded-full bg-orange-400/30 animate-float-delayed" />

      {/* Content */}
      <div className="relative z-10 max-w-5xl mx-auto px-6 text-center pt-24">
        {/* Badge */}
        <div className="animate-fade-in inline-flex items-center gap-2 px-4 py-2 rounded-full border border-glass-border bg-glass-white text-sm text-slate-700 mb-8">
          <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
          Now accepting new projects
        </div>

        {/* Headline */}
        <h1 className="font-display text-5xl sm:text-6xl lg:text-7xl font-extrabold leading-[1.1] tracking-tight text-slate-900 mb-6 animate-slide-up">
          Stunning Websites,{' '}
          <span className="gradient-text">Surprisingly Affordable</span>
        </h1>

        {/* Subheadline */}
        <p className="text-lg sm:text-xl text-slate-600 max-w-2xl mx-auto mb-10 leading-relaxed animate-slide-up-delayed opacity-0">
